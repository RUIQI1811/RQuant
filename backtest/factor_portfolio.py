"""Portfolio backtest orchestration for factor-research signals."""
...
from backtest.signal_portfolio import signal_frame_to_picks
...
DEFAULT_FACTOR_ENSEMBLE_OUTPUT = Path("data") / "portfolio_backtest_factor_ensemble"
...
def run_rank_ensemble_portfolio_backtest(
    *,
    data_dir: str | Path = "data/raw",
    metadata_path: str | Path | None = "config/stocklist.csv",
    output_dir: str | Path = DEFAULT_FACTOR_ENSEMBLE_OUTPUT,
    start_date: Optional[str] = None,
    end_date: Optional[str] = None,
    selection_config: RankEnsembleConfig,
    initial_cash: float = 10000000.0,
    hold_days: int = 20,
    commission_wan: float = 0.8,
    lot_size: int = 100,
    ...
    """Generate rank-ensemble signals and run the realistic next-open portfolio path."""
    ...
    signal_result = rank_factor_ensemble(
        factor_frame,
        config=config,
        filter_status=filter_status,
    )
    ...
    signal_output_dir = write_rank_ensemble_reports(
        signal_result,
        destination / "factor_signals",
        config=config,
    )
    ...
    portfolio_result.summary.update(
        {
            ...
            "factor_weights": dict(zip(config.factors, config.normalized_weights)),
