...
def make_next_open_returns(
    ...
    if missing:
        raise ValueError(f"missing required columns: {missing}")
    frame = prices[[date_col, symbol_col, open_col]].copy()
    frame[symbol_col] = frame[symbol_col].astype(str).str.zfill(6)
    frame[date_col] = pd.to_datetime(frame[date_col])
    frame = frame.sort_values([symbol_col, date_col]).reset_index(drop=True)
    if frame.duplicated([date_col, symbol_col]).any():
    ...
    grouped = frame.groupby(symbol_col, sort=False)[open_col]
    entry = grouped.shift(-1)
    for window in tuple(windows):
        if int(window) <= 0:
            raise ValueError("next-open return windows must be positive")
        exit_price = grouped.shift(-(int(window) + 1))
        frame[f"next_open_return_{int(window)}d"] = exit_price / entry - 1.0
    frame[date_col] = frame[date_col].dt.strftime("%Y-%m-%d")
    return frame.drop(columns=[open_col])
