...
from pipeline.factors.gtja191 import build_gtja191_panels, normalize_gtja_name  # noqa: E402
from pipeline.gtja191_batch import (  # noqa: E402
    GTJA191BatchConfig,
    GTJA191BatchRunner,
    parse_gtja_selection,
)
from pipeline.select_stock import load_raw_data  # noqa: E402
...
def main() -> None:
    ...
    if not factors:
        raise SystemExit("factor selection is empty after exclusions and lifecycle filtering")
    ...
    result = GTJA191BatchRunner(
        ...
        config=config,
        factor_statuses={normalize_gtja_name(name): statuses[name] for name in factors},
    ).run()
