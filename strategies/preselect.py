...
def load_raw_data(
    ...
    """读取 data_dir 下的 *.csv，可选按股票代码子集加载."""
    if not os.path.isdir(data_dir):
        raise FileNotFoundError(f"data_dir 不存在: {data_dir}")

    end_ts = pd.to_datetime(end_date) if end_date else None
    selected_symbols = (
        {str(symbol).split(".", 1)[0].zfill(6) for symbol in symbols}
    ...
    for fname in os.listdir(data_dir):
        ...
        if selected_symbols is not None and str(code).zfill(6) not in selected_symbols:
            continue
...
def _calc_warmup(cfg: dict, buffer: int) -> int:
    ...
    cfg_mbdsr = cfg.get("mbdsr", {})
    if cfg_mbdsr.get("enabled", False):
        warmup = max(warmup, 65 + buffer)

    cfg_confluence = cfg.get("bdsr_macd_obv", {})
    if cfg_confluence.get("enabled", False):
        warmup = max(
            warmup,
            int(cfg_confluence.get("bdsr_slow_window", 26)) + 1 + buffer,
            int(cfg_confluence.get("macd_slow_period", 26))
            + int(cfg_confluence.get("macd_signal_period", 9))
            + buffer,
            int(cfg_confluence.get("obv_ma_window", 20))
            + int(cfg_confluence.get("obv_trend_lookback", 3))
            + buffer,
        )
        ...
        try:
            ...
            if selector.vec_picks_from_prepared(pf, start=pick_date, end=pick_date):
                ...
                candidates.append(Candidate(
                    ...
                    extra={
                        "rci9": float(row["RCI9"]),
                        "rci26": float(row["RCI26"]),
                        "rci52": float(row["RCI52"]),
                        "signal_column": selector.signal_column,
                    },
                ))
        except Exception as exc:
            logger.debug("mBDSR skip %s: %s", code, exc)

    logger.info("mBDSR 选出: %d 只", len(candidates))
    return candidates


# =============================================================================
# BDSR + MACD + OBV 同日共振策略
# =============================================================================
...
def run_bdsr_macd_obv(
    prepared: Dict[str, pd.DataFrame],
    pick_date: pd.Timestamp,
    pool_codes: List[str],
    cfg_strategy: dict,
) -> List[Candidate]:
    """运行 BDSR 金叉、MACD 水上金叉、OBV 上升同日共振策略。"""
    selector = _make_bdsr_macd_obv_selector(cfg_strategy)
    date_str = pick_date.strftime("%Y-%m-%d")
    candidates: List[Candidate] = []
        ...
        try:
            ...
            if selector.vec_picks_from_prepared(pf, start=pick_date, end=pick_date):
                ...
                candidates.append(Candidate(
                    ...
                    turnover_n=float(row["turnover_n"]),
                    extra={
                        "bdsr_fast": float(row["BDSR_FAST"]),
                        "bdsr_slow": float(row["BDSR_SLOW"]),
            ...
            logger.debug("BDSR/MACD/OBV skip %s: %s", code, exc)

    logger.info("BDSR/MACD/OBV 共振选出: %d 只", len(candidates))
    return candidates
...
def run_preselect(
    *,
    ...
    if cfg.get("mbdsr", {}).get("enabled", False):
        all_candidates.extend(run_mbdsr(prepared, pick_ts, pool_codes, cfg["mbdsr"]))
